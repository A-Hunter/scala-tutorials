...
/**
  ...
  * Hence, it is bound to a new value each time multiplier is called. However, factor is not a formal parameter, then what is this?
  * Let us add one more line of code.
  * var factor = 3
  * val multiplier = (i:Int) => i * factor
  * Now factor has a reference to a variable outside the function but in the enclosing scope. The function references
  * factor and reads its current value each time.
